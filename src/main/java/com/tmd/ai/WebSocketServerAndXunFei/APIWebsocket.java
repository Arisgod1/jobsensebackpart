package com.tmd.ai.WebSocketServerAndXunFei;

import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.tmd.ai.WebSocketServerAndTongYi.realtimeAudio;
...
import static com.tmd.ai.WebSocketServerAndXunFei.realTime.message1;
import static com.tmd.ai.WebSocketServerAndXunFei.realTime.message2;
...
public class APIWebsocket {
    ...
    public APIWebsocket(@Qualifier("chatClient") ChatClient chatClient) {
        com.tmd.ai.WebSocketServerAndTongYi.APIWebsocket.chatClient = chatClient;
    }
        ...
        executor.submit(new Runnable() {
            ...
            /*if(message.isEmpty()){
                ...
                try {
                    ...
                    if (head.getString("action").equals("started")) {
                        task_id =task_id+1;
                        log.info("[开始识别]，服务器知道任务已经开启");
                        // 开始识别，调用sendmessage发送消息，激活下面的锁
                      //  log.info("激活锁，准备发送数据，task_id为{}", string1);
//                        flag = true;
                    } else if (head.getString("action").equals("result")) {
                        synchronized (message2){
                            log.info("结果开始生成,开始接受");
                            String string1 = head.getString("data");
                            //这个是json类型的字符串
                            JSONObject jsonObject = JSONObject.parseObject(string1);
                            JSONObject cn = jsonObject.getJSONObject("cn");
                            JSONObject st = cn.getJSONObject("st");
                            String string2 = st.getString("type");
                            if (string2.equals("0")) {
                                JSONArray rt = st.getJSONArray("rt");
                                JSONObject jsonObject1 = rt.getJSONObject(0);
                                JSONArray ws = jsonObject1.getJSONArray("ws");
                                for (int i = 0; i < ws.size(); i++) {
                                    JSONObject jsonObject2 = ws.getJSONObject(i);
                                    JSONArray jsonArray = jsonObject2.getJSONArray("cw");
                                    JSONObject jsonObject3 = jsonArray.getJSONObject(0);
                                    String string3 = jsonObject3.getString("w");
                                    message1 += string3;
                                    log.info("[实时语音识别结果] {}", message1);
                                }
                            }
                            message2.notify();
                        }
        ...
        try {
            ...
            try {
                            ...
                            if (!task_id.isEmpty()){
                                ...
                                try {
                                    ...
                                    if (message.hasRemaining()) { // 等价于 remaining() > 0
                                        ...
                                        // 新增：验证WebSocket连接状态
                                        if (com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session == null || !com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.isOpen()) {
                                        ...
                                        String connectionInfo = String.format(
                                                ...
                                                com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.isOpen() ? "已连接" : "已关闭",
                                                com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.getMaxBinaryMessageBufferSize()
                                        );
                                        ...
                                        try{
                                            ...
                                            // 获取详细的网络错误信息
                                            if (com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.getUserProperties().containsKey("javax.websocket.endpoint.remoteAddress")) {
                                                Socket socket = (Socket) com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.getUserProperties().get("javax.websocket.endpoint.remoteAddress");
                                                if (socket != null) {
                ...
                executor.submit(new Runnable() {
                    @Override
                    public void run() {
                        String json ="{\"end\":true}";
                        if(flag){
                            flag = false;
                            try {
                                log.info("[发送消息] 尝试发送finish-task数据: {}", json);
                                com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.getBasicRemote().sendText(json);
                                    log.info("[任务完成]");
                                    //任务可以结束了，关闭链接
                                    // APIWebsocket.class.notify();
                                    synchronized (realTime.message3){
                                        ...
                                        String response = chatClient.prompt()
                                                ...
                                                .call()
                                                .content();
                                        realTime.response=response;
                                        log.info("[结果消息]{}", response);
                                        count++;
                                        realTime.message3.notify();
                                    }
                                    if(com.tmd.ai.WebSocketServerAndTongYi.APIWebsocket.count==5){
                                        session.close();
                                    ...
                                    String runTaskMessage1 = "{\"header\":{\"action\":\"run-task\",\"task_id\":\""+string+"\",\"streaming\":\"duplex\"},\"payload\":{\"task_group\":\"audio\"" +
                                            ",\"task\":\"asr\",\"function\":\"recognition\",\"model\":\"paraformer-realtime-v2\",\"parameters\":{\"format\":\"pcm\",\"sample_rate\":16000,\"disfluency_removal_enabled\":false,\"language_hints\":[\"zh\"]}" +
                                            ",\"input\":{}}}";
                                    com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session.getBasicRemote().sendText(runTaskMessage1);
    ...
    public Map<String, Object> connect(){
        try {
            if((session== null||!session.isOpen())) {
                URIBuilder builder = new URIBuilder("wss://rtasr.xfyun.cn/v1/ws");
                builder.addParameter("appid",这里填你的 );
                builder.addParameter("ts", String.valueOf(System.currentTimeMillis()));
                String baseString=这里填你的+System.currentTimeMillis();
                //MD5加密
                String signaOrigin = DigestUtils.md5Hex(baseString);
                //以apikey为key对上面的字符串进行HmacSHA1，再对加密后的字符串进行base64编码
                String signa = HmacUtils.hmacSha1Hex(这里填你的apiKey, signaOrigin);
                builder.addParameter("signa", signa);
                URI build = builder.build();
                WebSocketContainer container = ContainerProvider.getWebSocketContainer();
                log.info("container容器为{}", container);
                Session session = container.connectToServer(this, build);
                log.info("session为{}", session);
                com.tmd.ai.WebSocketServerAndXunFei.APIWebsocket.session = session;
                count++;
            }
